(() => {
  ...
  function observeNode(node, closestSelector) {
    ...
    if (7 & FEATURE_CLOSEST_SELECTOR && closestSelector) {
      if (closestSelector === "parent") {
        nodeToObserve = node.parentNode;
      } else {
        nodeToObserve = node.closest(closestSelector);
      }
      7 & FEATURE_RESIZE_OBSERVER && 7 & FEATURE_CLOSEST_SELECTOR && storeReference(nodeToObserve, node);
    }
    if (7 & FEATURE_RESIZE_OBSERVER) {
      if (!resizeObserver) {
