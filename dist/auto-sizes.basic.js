(() => {
  ...
  function observeNode(node, closestSelector) {
    ...
    if (0 & FEATURE_CLOSEST_SELECTOR && closestSelector) {
      if (closestSelector === "parent") {
        nodeToObserve = node.parentNode;
      } else {
        nodeToObserve = node.closest(closestSelector);
      }
      0 & FEATURE_RESIZE_OBSERVER && 0 & FEATURE_CLOSEST_SELECTOR && storeReference(nodeToObserve, node);
    }
    if (0 & FEATURE_RESIZE_OBSERVER) {
      if (!resizeObserver) {
