(() => {
  ...
  function observeNode(node, closestSelector) {
    ...
    if (5 & FEATURE_CLOSEST_SELECTOR && closestSelector) {
      if (closestSelector === "parent") {
        nodeToObserve = node.parentNode;
      } else {
        nodeToObserve = node.closest(closestSelector);
      }
      5 & FEATURE_RESIZE_OBSERVER && 5 & FEATURE_CLOSEST_SELECTOR && storeReference(nodeToObserve, node);
    }
    if (5 & FEATURE_RESIZE_OBSERVER) {
      if (!resizeObserver) {
